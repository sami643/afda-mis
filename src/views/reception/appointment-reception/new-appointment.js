...
import { newAppointmentAPi, directprateListAPI } from "../../../api/utils";
import { newAppointmentValidationSchema } from "../../data/validation";
import NewAppoinmentPhoto from "./../../../assets/images/meeting.png";
...
const Appointment = (props) => {
  ...
  const newAppointment = async (values) => {
    ...
    try {
      const response = await newAppointmentAPi(data);
      if (response.data.message === "New Appointment is Created") {
        ...
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <h4 className="mb-1">
                <img
                  src={NewAppoinmentPhoto}
                  width={40}
                  className="mx-1 mt-2 mb-0"
                />
                د نوي ملاقات/ لیدنې ثبت{" "}
              </h4>
            </CCardHeader>
